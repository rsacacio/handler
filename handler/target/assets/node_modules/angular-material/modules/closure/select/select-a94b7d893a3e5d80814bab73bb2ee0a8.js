var ng={};ng.material={};ng.material.components={};var SELECT_EDGE_MARGIN=8;var selectNextId=0;angular.module("material.components.select",["material.core","material.components.backdrop"]).directive("mdSelect",SelectDirective).directive("mdSelectMenu",SelectMenuDirective).directive("mdOption",OptionDirective).directive("mdOptgroup",OptgroupDirective).provider("$mdSelect",SelectProvider);
function SelectDirective($mdSelect,$mdUtil,$mdTheming,$mdAria,$interpolate,$compile,$parse){return{restrict:"E",require:["mdSelect","ngModel","?^form"],compile:compile,controller:function(){}};function compile(element,attr){var labelEl=element.find("md-select-label").remove();if(!labelEl.length)labelEl=angular.element("<md-select-label><span></span></md-select-label>");else if(!labelEl[0].firstElementChild){var spanWrapper=angular.element("<span>");spanWrapper.append(labelEl.contents());labelEl.append(spanWrapper)}labelEl.append('<span class="md-select-icon" aria-hidden="true"></span>');
labelEl.addClass("md-select-label");if(!labelEl[0].hasAttribute("id"))labelEl.attr("id","select_label_"+$mdUtil.nextUid());if(!element.find("md-content").length)element.append(angular.element("<md-content>").append(element.contents()));if(attr.mdOnOpen)element.find("md-content").prepend(angular.element("<md-progress-circular>").attr("md-mode","indeterminate").attr("ng-hide","$$loadingAsyncDone").wrap("<div>").parent());if(attr.name){var autofillClone=angular.element('<select class="md-visually-hidden">');
autofillClone.attr({"name":"."+attr.name,"ng-model":attr.ngModel,"aria-hidden":"true","tabindex":"-1"});var opts=element.find("md-option");angular.forEach(opts,function(el){var newEl=angular.element("<option>"+el.innerHTML+"</option>");if(el.hasAttribute("ng-value"))newEl.attr("ng-value",el.getAttribute("ng-value"));else if(el.hasAttribute("value"))newEl.attr("value",el.getAttribute("value"));autofillClone.append(newEl)});element.parent().append(autofillClone)}var selectTemplate='<div class="md-select-menu-container">'+
"<md-select-menu "+(angular.isDefined(attr.multiple)?"multiple":"")+">"+element.html()+"</md-select-menu></div>";element.empty().append(labelEl);attr.tabindex=attr.tabindex||"0";return function postLink(scope,element,attr,ctrls){var isOpen;var isDisabled;var mdSelectCtrl=ctrls[0];var ngModel=ctrls[1];var formCtrl=ctrls[2];var labelEl=element.find("md-select-label");var customLabel=labelEl.text().length!==0;var selectContainer,selectScope,selectMenuCtrl;createSelect();$mdTheming(element);if(attr.name&&
formCtrl){var selectEl=element.parent()[0].querySelector('select[name=".'+attr.name+'"]');formCtrl.$removeControl(angular.element(selectEl).controller())}var originalRender=ngModel.$render;ngModel.$render=function(){originalRender();syncLabelText()};mdSelectCtrl.setLabelText=function(text){if(customLabel)return;mdSelectCtrl.setIsPlaceholder(!text);text=text||attr.placeholder||"";var target=customLabel?labelEl:labelEl.children().eq(0);target.text(text)};mdSelectCtrl.setIsPlaceholder=function(val){val?
labelEl.addClass("md-placeholder"):labelEl.removeClass("md-placeholder")};scope.$$postDigest(function(){setAriaLabel();syncLabelText()});function setAriaLabel(){var labelText=element.attr("placeholder");if(!labelText)labelText=element.find("md-select-label").text();$mdAria.expect(element,"aria-label",labelText)}function syncLabelText(){if(selectContainer){selectMenuCtrl=selectMenuCtrl||selectContainer.find("md-select-menu").controller("mdSelectMenu");mdSelectCtrl.setLabelText(selectMenuCtrl.selectedLabels())}}
...
var selectEl=selectContainer.find("md-select-menu");selectEl.data("$ngModelController",ngModel);selectEl.data("$mdSelectController",mdSelectCtrl);selectScope=scope.$new();selectContainer=$compile(selectContainer)(selectScope);selectMenuCtrl=selectContainer.find("md-select-menu").controller("mdSelectMenu")}function handleKeypress(e){var allowedCodes=[32,13,38,40];if(allowedCodes.indexOf(e.keyCode)!=-1){e.preventDefault();openSelect(e)}else if(e.keyCode<=90&&e.keyCode>=31){e.preventDefault();var node=
selectMenuCtrl.optNodeForKeyboardSearch(e);if(!node)return;var optionCtrl=angular.element(node).controller("mdOption");if(!selectMenuCtrl.isMultiple)selectMenuCtrl.deselect(Object.keys(selectMenuCtrl.selected)[0]);selectMenuCtrl.select(optionCtrl.hashKey,optionCtrl.value);selectMenuCtrl.refreshViewValue();ngModel.$render()}}function openSelect(){scope.$evalAsync(function(){isOpen=true;$mdSelect.show({scope:selectScope,preserveScope:true,skipCompile:true,element:selectContainer,target:element[0],hasBackdrop:true,
loadingAsync:attr.mdOnOpen?scope.$eval(attr.mdOnOpen)||true:false}).then(function(selectedText){isOpen=false})})}}}}SelectDirective.$inject=["$mdSelect","$mdUtil","$mdTheming","$mdAria","$interpolate","$compile","$parse"];
function SelectMenuDirective($parse,$mdUtil,$mdTheming){SelectMenuController.$inject=["$scope","$attrs","$element"];return{restrict:"E",require:["mdSelectMenu","?ngModel"],controller:SelectMenuController,link:{pre:preLink}};function preLink(scope,element,attr,ctrls){var selectCtrl=ctrls[0];var ngModel=ctrls[1];$mdTheming(element);element.on("click",clickListener);element.on("keypress",keyListener);if(ngModel)selectCtrl.init(ngModel);configureAria();function configureAria(){element.attr({"id":"select_menu_"+
...
opts){if(!opts.target)throw new Error("$mdSelect.show() expected a target element in options.target but got "+'"'+opts.target+'"!');angular.extend(opts,{isRemoved:false,target:angular.element(opts.target),parent:angular.element(opts.parent),selectEl:element.find("md-select-menu"),contentEl:element.find("md-content"),backdrop:opts.hasBackdrop&&angular.element('<md-backdrop class="md-select-backdrop md-click-catcher">')});opts.resizeFn=function(){$$rAF(function(){$$rAF(function(){animateSelect(scope,
element,opts)})})};angular.element($window).on("resize",opts.resizeFn);angular.element($window).on("orientationchange",opts.resizeFn);configureAria();element.removeClass("md-leave");var optionNodes=opts.selectEl[0].getElementsByTagName("md-option");if(opts.loadingAsync&&opts.loadingAsync.then)opts.loadingAsync.then(function(){scope.$$loadingAsyncDone=true;$$rAF(function(){$$rAF(function(){if(opts.isRemoved)return;animateSelect(scope,element,opts)})})});else if(opts.loadingAsync)scope.$$loadingAsyncDone=
true;if(opts.disableParentScroll&&!$mdUtil.getClosest(opts.target,"MD-DIALOG"))opts.restoreScroll=$mdUtil.disableScrollAround(opts.target);else opts.disableParentScroll=false;$timeout(activateInteraction,75,false);if(opts.backdrop){$mdTheming.inherit(opts.backdrop,opts.parent);opts.parent.append(opts.backdrop)}opts.parent.append(element);$$rAF(function(){$$rAF(function(){if(opts.isRemoved)return;animateSelect(scope,element,opts)})});return $mdUtil.transitionEndPromise(opts.selectEl,{timeout:350});
function configureAria(){opts.target.attr("aria-expanded","true")}function activateInteraction(){if(opts.isRemoved)return;var selectCtrl=opts.selectEl.controller("mdSelectMenu")||{};element.addClass("md-clickable");opts.backdrop&&opts.backdrop.on("click",function(e){e.preventDefault();e.stopPropagation();opts.restoreFocus=false;scope.$apply($mdSelect.cancel)});opts.selectEl.on("keydown",function(ev){switch(ev.keyCode){case $mdConstant.KEY_CODE.SPACE:case $mdConstant.KEY_CODE.ENTER:var option=$mdUtil.getClosest(ev.target,
"md-option");if(option){opts.selectEl.triggerHandler({type:"click",target:option});ev.preventDefault()}break;case $mdConstant.KEY_CODE.TAB:case $mdConstant.KEY_CODE.ESCAPE:ev.preventDefault();opts.restoreFocus=true;scope.$apply($mdSelect.cancel)}});opts.selectEl.on("keydown",function(ev){switch(ev.keyCode){case $mdConstant.KEY_CODE.UP_ARROW:return focusPrevOption();case $mdConstant.KEY_CODE.DOWN_ARROW:return focusNextOption();default:if(ev.keyCode>=31&&ev.keyCode<=90){var optNode=opts.selectEl.controller("mdSelectMenu").optNodeForKeyboardSearch(ev);
optNode&&optNode.focus()}}});function focusOption(direction){var optionsArray=nodesToArray(optionNodes);var index=optionsArray.indexOf(opts.focusedNode);if(index===-1)index=0;else if(direction==="next"&&index<optionsArray.length-1)index++;else if(direction==="prev"&&index>0)index--;var newOption=opts.focusedNode=optionsArray[index];newOption&&newOption.focus()}function focusNextOption(){focusOption("next")}function focusPrevOption(){focusOption("prev")}opts.selectEl.on("click",checkCloseMenu);opts.selectEl.on("keydown",
function(e){if(e.keyCode==32||e.keyCode==13)checkCloseMenu()});function checkCloseMenu(){if(!selectCtrl.isMultiple){opts.restoreFocus=true;scope.$evalAsync(function(){$mdSelect.hide(selectCtrl.ngModel.$viewValue)})}}}}function onRemove(scope,element,opts){opts.isRemoved=true;element.addClass("md-leave").removeClass("md-clickable");opts.target.attr("aria-expanded","false");angular.element($window).off("resize",opts.resizeFn);angular.element($window).off("orientationchange",opts.resizefn);opts.resizeFn=
undefined;var mdSelect=opts.selectEl.controller("mdSelect");if(mdSelect)mdSelect.setLabelText(opts.selectEl.controller("mdSelectMenu").selectedLabels());return $mdUtil.transitionEndPromise(element,{timeout:350}).then(function(){element.removeClass("md-active");opts.backdrop&&opts.backdrop.remove();if(element[0].parentNode===opts.parent[0])opts.parent[0].removeChild(element[0]);if(opts.disableParentScroll)opts.restoreScroll();if(opts.restoreFocus)opts.target.focus()})}function animateSelect(scope,
element,opts){var containerNode=element[0],targetNode=opts.target[0].firstElementChild.firstElementChild,parentNode=opts.parent[0],selectNode=opts.selectEl[0],contentNode=opts.contentEl[0],parentRect=parentNode.getBoundingClientRect(),targetRect=targetNode.getBoundingClientRect(),shouldOpenAroundTarget=false,bounds={left:parentRect.left+SELECT_EDGE_MARGIN,top:SELECT_EDGE_MARGIN,bottom:parentRect.height-SELECT_EDGE_MARGIN,right:parentRect.width-SELECT_EDGE_MARGIN-($mdUtil.floatingScrollbars()?16:0)},
...
containerNode.style.left=clamp(bounds.left,left,bounds.right-containerRect.width)+"px";containerNode.style.top=clamp(bounds.top,top,bounds.bottom-containerRect.height)+"px";selectNode.style[$mdConstant.CSS.TRANSFORM_ORIGIN]=transformOrigin;selectNode.style[$mdConstant.CSS.TRANSFORM]="scale("+Math.min(targetRect.width/selectMenuRect.width,1)+","+Math.min(targetRect.height/selectMenuRect.height,1)+")";$$rAF(function(){element.addClass("md-active");selectNode.style[$mdConstant.CSS.TRANSFORM]="";if(focusedNode){opts.focusedNode=
focusedNode;focusedNode.focus()}})}}function clamp(min,n,max){return Math.max(min,Math.min(n,max))}function getOffsetRect(node){return node?{left:node.offsetLeft,top:node.offsetTop,width:node.offsetWidth,height:node.offsetHeight}:{left:0,top:0,width:0,height:0}}}SelectProvider.$inject=["$$interimElementProvider"];function nodesToArray(nodes){var results=[];for(var i=0;i<nodes.length;++i)results.push(nodes.item(i));return results}ng.material.components.select=angular.module("material.components.select");